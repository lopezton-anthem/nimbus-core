...
export class TreeGrid extends BaseTableElement implements ControlValueAccessor {
    ...
    getNestedCollectionParamConfigs(collectionParams: CollectionParams, param: Param, rowNode: any): ParamConfig[] {
        ...
        for (const key in this.element.tableBasedData.collectionParams[rowNode.data.elemId]) {
            if (this.element.tableBasedData.collectionParams[rowNode.data.elemId].hasOwnProperty(key)) {
                const element = this.element.tableBasedData.collectionParams[rowNode.data.elemId][key];
